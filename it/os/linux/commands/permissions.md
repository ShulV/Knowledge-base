...
после смены нужно перезагрузить сессию пользователя. Выйти/войти.
Можно выполнить
```bash
newgrp docker
# Она **открывает новую shell-сессию**, где твоя **основная группа** временно становится `docker`. Полезно, т.к. изменения в группах часто не подхватываются в уже открытом терминале
```
После этого выполнить, чтобы убедиться, что у пользователя добавилась группа
```bash
groups
```
